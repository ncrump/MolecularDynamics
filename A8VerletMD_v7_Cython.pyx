"""
Created on Sat Apr 18 21:57:54 2015
...
# Plots adjusted for multiple temps
# Velocities output to file for analysis
...
cpdef SimRun():
    ...
    cdef int    i,ntmp,eql,prd,neql,nstp,cell,nprt,step,hbins,gbins,vstore,vtimes,vcnt
    ...
    cdef double tmp,tmpP,tmpK,tmpE,stdP,stdK,stdE,vmax,dv,dr,uLR,vLR,vtemp1,vtemp2
    ...
    dt     = 0.005    # time step
    a      = 1.788    # side length of lattice
    cell   = 3        # translations of unit cell
    hbins  = 20       # number of bins for H-function
    gbins  = 120      # number of bins for g(r)

    # velocity storage
    vstore = 1        # steps between storing velocities
    vtimes = 500      # number of steps to store velocities
    vtemp1 = 1.5      # first temp to store velocities
    vtemp2 = 0.1      # second temp to store velocities
    # velocity files
    fname1 = 'velocity_rho0.7_temp1.5.txt'
    fname2 = 'velocity_rho0.7_temp0.1.txt'

    # system run temperatures
    #temp  = [2.00,1.50,1.00,0.80,0.75,0.70,0.65,0.60,0.50,0.30,0.10]
    temp = [1.5,0.1]
    #----------------------------------------------
    ...
    for tmp in temp:
        ...
        while step < nstp:
            ...
            if step == eql:
                ...
                if tmp == vtemp1 or vtemp2:
                    vels = [vsqr**0.5]
                    tvel = [[(step-eql)*dt]*nprt]
            ...
            if step >= eql:
                ...
                if step < eql+vtimes:
                    if tmp == vtemp1 or vtemp2:
                        vcnt += 1
                        if vcnt == vstore:
                            vels.append(vsqr**0.5)
                            tvel.append([(step-eql+1)*dt]*nprt)
                            vcnt = 0
        ...
        if tmp == vtemp1:
            tvel = np.array(tvel).flatten()
            vels = np.array(vels).flatten()
            stack = np.column_stack((tvel,vels))
            np.savetxt(fname1,stack,fmt=('%12.6f','%12.6f'))
            print 'output file:',fname1
        if tmp == vtemp2:
            tvel = np.array(tvel).flatten()
            vels = np.array(vels).flatten()
            stack = np.column_stack((tvel,vels))
            np.savetxt(fname2,stack,fmt=('%12.6f','%12.6f'))
            print 'output file:',fname2
    ...
    box = axP1.get_position()
    axP1.set_position([box.x0,box.y0,box.width*buf,box.height])
    axP2 = figP.add_subplot(212)
    box = axP2.get_position()
    axP2.set_position([box.x0,box.y0,box.width*buf,box.height])
    figK = plt.figure()
    axK1 = figK.add_subplot(211)
    box = axK1.get_position()
    axK1.set_position([box.x0,box.y0,box.width*buf,box.height])
    axK2 = figK.add_subplot(212)
    box = axK2.get_position()
    axK2.set_position([box.x0,box.y0,box.width*buf,box.height])
    figE = plt.figure()
    axE1 = figE.add_subplot(211)
    box = axE1.get_position()
    axE1.set_position([box.x0,box.y0,box.width*buf,box.height])
    axE2 = figE.add_subplot(212)
    box = axE2.get_position()
    axE2.set_position([box.x0,box.y0,box.width*buf,box.height])
    figT = plt.figure()
    axT1 = figT.add_subplot(211)
    box = axT1.get_position()
    axT1.set_position([box.x0,box.y0,box.width*buf,box.height])
    axV1 = figT.add_subplot(212)
    box = axV1.get_position()
    axV1.set_position([box.x0,box.y0,box.width*buf,box.height])
    figO = plt.figure()
    axO1 = figO.add_subplot(111)
    box = axO1.get_position()
    axO1.set_position([box.x0,box.y0,box.width*buf,box.height])
    figH = plt.figure()
    axH1 = figH.add_subplot(111)
    box = axH1.get_position()
    axH1.set_position([box.x0,box.y0,box.width*buf,box.height])
    figG = plt.figure()
    axG1 = figG.add_subplot(111)
    box = axG1.get_position()
    axG1.set_position([box.x0,box.y0,box.width*buf,box.height])
    ...
    for i in range(ntmp):
        ...
        axP1.legend(loc=loc1,bbox_to_anchor=(1.02,0.5),fontsize=13)
        ...
        axK1.legend(loc=loc1,bbox_to_anchor=(1.02,0.5),fontsize=13)
        ...
        axE1.legend(loc=loc1,bbox_to_anchor=(1.02,0.5),fontsize=13)
        ...
        axT1.legend(loc=loc1,bbox_to_anchor=(1.02,0.5),fontsize=13)
